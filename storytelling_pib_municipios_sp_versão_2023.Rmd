...
dados_grafico_1 %>%
  ...
  geom_text(data = tibble(valor = 685, municipio = "Barueri " ),
            aes(label = str_wrap( paste0( scales::percent(proporcao_capital, accuracy = 1.0)," do total do PIB paulista") ,12),),
            color = cor_texto,
            fontface = "bold",
            vjust = 0.8)+
  ...
  labs(
    title = str_wrap("Vinte municípios com capital", 16) ,
    subtitle = "",
    x= "",
    y= ""
  )
...
dados_grafico_2 %>%
  ...
  labs(
    title =  str_wrap("Vinte municípios sem capital", 16) ,
    subtitle = "",
    x= "",
    y= ""
  )
...
graf1 + graf2 + 
  plot_annotation(title = "PIB dos municípios de São Paulo em 2023", 
                  ...
                  caption = "Fonte: IBGE. Elaboração: VPR/DIESO",
                  theme = theme(
                    plot.background = element_rect(fill =cor_fundo),
